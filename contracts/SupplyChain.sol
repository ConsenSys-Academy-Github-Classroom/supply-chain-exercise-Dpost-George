...
contract SupplyChain {
    ...
    modifier isOwner() {
        require(
            msg.sender == owner,
            "Not Owner of this contract, please contact the Owner!"
        );
        _;
    }
    ...
    //  if(items[sku].seller != msg.sender) {revert("Not a Seller")}
    ...
    modifier forSale(uint256 _sku) {
        require(items[_sku].state == State.ForSale, "The Item is for sale!");
        _;
    }
    //modifier Sold
    ...
    modifier shipped(uint256 _sku) {
        require(items[_sku].state == State.Shipped, "Item is schipped");
        _;
    }
    // modifier received(uint _sku)
    modifier received(uint256 _sku) {
        require(items[_sku].state == State.Received, "Item is Recieved");
        _;
    }
    ...
    constructor() public {
        ...
        skuCount = 0; // per default when declared the value is 0
    }
    ...
    {
        if (items[sku].seller != msg.sender) {
